...
from . import (YAML_CONTENT, MODEL_TYPE_NAME, TEMPLATE_TYPE_NAME, MODEL_FILE,
               TEMPLATE_FILE, DEFAULT_TEST_FIELD_NAME,
               DEFAULT_TEST_FIELD_CONTENT, JSON_CONTENT)


class TestRawLoading():
    #pylint: disable=too-many-locals
    @staticmethod
    def dependency_test(data_core, file_structure, raw_content, raw_extension):
        ...
        model_raw_dir = f"test.{raw_extension}"
...
class TestTemplate2(Template, dependencies=["TestTemplate",],
                    core_name="{core_name}"):
    name: str
        """.strip()

        model_raw = file_structure["model_raw_dir"].join(model_raw_dir)
        model_raw.write(raw_content)

        raw_template = raw_templates_dir.join(f"{TEMPLATE_TYPE_NAME}." +
                                              raw_extension)
        raw_template2 = raw_templates_dir.join(f"{TEMPLATE_TYPE_NAME}2." +
                                               raw_extension)

        raw_template.write(raw_content)
        raw_template2.write(raw_content)

        file_structure["models_dir"].join(model_module_name +
                                          ".py").write(model_content)
        file_structure["templates_dir"].join(template_module_name +
                                             ".py").write(template_content)
        file_structure["templates_dir"].join(f"{template_module_name}2" +
                                             ".py").write(template_file2)

        assert len(
            data_core.recursively_add_module(
                file_structure["models_dir"])) == 1
        assert len(data_core.recursively_add_module(
            file_structure["templates_dir"])) == 2

        data_core.recursively_instance_template(
            file_structure["raw_templates_dir"])
        ...
        data_core.recursively_instance_model(
            file_structure["root_model_raw_dir"])
    ...
    @staticmethod
    def raw_testing(data_core, file_structure, raw_content, raw_extension):
        pwetty = pprint.PrettyPrinter()
        pwetty.pprint(file_structure)
        ...
        model_raw = file_structure["model_raw_dir"].join(
            f"test.{raw_extension}")
        model_raw.write(raw_content)

        raw_template = file_structure["raw_templates_dir"].join(
            f"{TEMPLATE_TYPE_NAME}." f"{raw_extension}")
        raw_template.write(raw_content)

        file_structure["models_dir"].join(
            model_module_name).write(model_content)
        file_structure["templates_dir"].join(
            template_module_name).write(template_content)
        ...
        for current_model_instance in data_core.all_model_instances:
            for template_instance in data_core.all_template_instances:
                ...
                model_parent = current_model_instance.parents.get(
                    template_name)
                field_content = getattr(
                    template_instance, DEFAULT_TEST_FIELD_NAME)
