...
class DataType(TinyDB):
    ...
    def __new__(cls, *_, db_file: Optional['panda_core_data.PathType'] = None,
        ...
        necessary because dataclasses create a custom __init__ method. Which we
        doesn't use at all if a raw file is supplied.
        """
        if not hasattr(cls, "dataclass_args"):
            raise PCDNeedsToBeInherited("You can`t create a DataType instance "
                                        "directly without instancing it first.")
    ...
    def __repr__(self) -> str:
        ...
        fields_list = [
            "\t\t{}({}) = {}".format(
                ...
                getattr(
                    self,
                    current_field.name)) for current_field in fields_list]
