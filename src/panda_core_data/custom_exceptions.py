'''Module containing all custom exceptions used by the package. All exceptions
related to this package has the PCD prefix.

Please, if you used one of our methods and it raised an exception that doesn't
have our PCD prefix **and** it came from inside our files send us a ticket in
this link:
...
class PCDNeedsToBeInherited(PCDException):
    """Exception raised if a :class:`~panda_core_data.models.Model` or
    :class:`~panda_core_data.models.Template` would be called without being
    inherited fist"""


class PCDDuplicatedTypeName(PCDException):
    """Exception raised if a :class:`~panda_core_data.models.Model` or
    :class:`~panda_core_data.models.Template` Type already exists with the same
    name"""
...
class PCDKeyError(KeyError, PCDException):
    "Same as :class:`KeyError`, but with :class:`PCDException`"
